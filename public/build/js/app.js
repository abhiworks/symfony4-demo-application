...
/* WEBPACK VAR INJECTION */(function($) {$(document).ready(function () {
  for (i = 0; i < sessionStorage.length; i++) {
    // Get all previous selected values from the session
  ...
  $("select").on("change", function () {
    if ($(" option[value=" + $(this).val() + " ]").attr('data-color') != '') {
      // Set bg color of square
  ...
  function hideIfNotValid(index, value) {
    ...
    $.each(valOptions, function (optionsindex, optionsvalue) {
      ...
      if ($.inArray($('#configurator_item_' + configVals[0] + ' option:selected').val(), configVals[1]) != -1) {} else {
        // Hide the option if not valid
        if ($(" option[value=" + index + "]").parent().get(0).tagName != 'SPAN') $(" option[value=" + index + "]").wrap('<span class="configurator_Profile"/>');
      }
    });
  }
});
